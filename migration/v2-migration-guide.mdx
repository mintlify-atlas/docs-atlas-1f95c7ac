...
description: 'Migrate from v1.x to v2.0 with fee collection support and new features'
---
...
The fee parameter structure has been completely redesigned in v2.x:
...
<Warning>
**Removed Types:** `TakingFeeInfo` has been removed. Use `IntegratorFeeRequest` instead.
</Warning>
...
```typescript v1.x
// zkSync had custom `create2` prefix and bytecode hash
// for address calculation
```

```typescript v2.x
// zkSync uses the standard `EscrowFactory` implementation
// (same as Ethereum, Polygon, etc.)
```
...
<Note>
Only `SrcEscrowCreated` topic changed due to the new `bytes parameters` field in the Immutables struct.
</Note>
...
<Steps>
  <Step title="Update event listeners">
    Handle both topic hashes for `SrcEscrowCreated`:
  ...
  <Step title="Decode events based on topic">
    ```typescript
  ...
  <Step title="Filter orders by version">
    When querying APIs:
  ...
  <Step title="Update escrow address computation">
  ...
  <Step title="Update contract addresses">
    If you have hardcoded escrow factory or implementation addresses, update them to the v2.x addresses listed above.
  </Step>
</Steps>
